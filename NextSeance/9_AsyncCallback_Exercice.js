...
// async/Await
const promise112 = (operationName, callback) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      resolve(operationName);
    }, 3000);
...
const list_elemine = document.getElementById("list_elemine");
...
createCondidatePromisePromise("MONDHER")
  ...
  // ASYNC & AWAIT
  async () => {
    const { newCandidate } = await createCondidatePromise("mondher");
    abbi();
...
// Get Todos
(async () => {
  const todos = await fetch("https://jsonplaceholder.typicode.com/posts").then(
    (res) => res.json()
  );
  console.log(todos);
})();
